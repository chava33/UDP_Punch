...
int main(int argc, char* argv[])
{
    ...
    // as well.
    if (sendto(s, "hi, i am bitcircle PC", BUFLEN, 0, (struct sockaddr*)(&si_other), slen)==-1)
        diep("sendto");
        ...
        if (server.host == si_other.sin_addr.s_addr && server.port == (short)(si_other.sin_port))
        {
            ...
            for (k = 0; k < 2; k++)
            {
                ...
                for (i = 0; i < n; i++)
                {
                    ...
                    // data in here.
                    if (sendto(s, "hi again, i am bitcircle PC", BUFLEN, 0, (struct sockaddr*)(&si_other), slen)==-1)
                        diep("sendto()");
                }
            }
        }
